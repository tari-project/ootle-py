...
def send_sealed(
    ...
    *before* the transaction is submitted, so a fast finalisation cannot
    ...
    tx_id = transport.submit_transaction(envelope)
    return PendingTransaction(tx_id=tx_id, _transport=transport, _watcher=watcher, _timeout=timeout)
...
def send_dry_run(
    ...
    *,
    ...
    envelope = crypto.bor_encode_transaction(sealed.json)
    return transport.submit_transaction_dry_run(envelope)
