"""Free-function endpoint helpers for :class:`IndexerTransport`.
...
def submit_transaction(transport: IndexerTransport, envelope_b64: str) -> TransactionId:
    """``POST /transactions`` → :class:`TransactionId`."""
    resp = _post(transport, "transactions", {"transaction": envelope_b64})
    return parse_json_object(resp, parse_submit_response)


def submit_transaction_dry_run(transport: IndexerTransport, envelope_b64: str) -> DryRunResult:
    """``POST /transactions/dry-run`` → :class:`DryRunResult`."""
    resp = _post(transport, "transactions/dry-run", {"transaction": envelope_b64})
...
def get_transaction_result(
    transport: IndexerTransport, tx_id: TransactionId
    ...
    """
    resp = _get(transport, f"transactions/{tx_id}/result")
...
def get_transaction_receipt(
    ...
    """
    resp = transport._request(  # pyright: ignore[reportPrivateUsage]  # internal access
...
def get_transaction_watcher(
    ...
    if existing is not None:
        return existing
    from ootle._sync._watcher import TransactionWatcher  # noqa: PLC0415

    watcher = TransactionWatcher(transport.client, transport.url)
    try:
        watcher.start()
...
def _post(transport: IndexerTransport, path: str, payload: dict[str, Any]) -> httpx.Response:
    resp = transport._request("POST", path, json=payload)  # pyright: ignore[reportPrivateUsage]  # internal access
    assert resp is not None  # noqa: S101 — `allow_404=False`
    return resp


def _get(transport: IndexerTransport, path: str) -> httpx.Response:
    resp = transport._request("GET", path)  # pyright: ignore[reportPrivateUsage]  # internal access
    assert resp is not None  # noqa: S101 — `allow_404=False`
    return resp
