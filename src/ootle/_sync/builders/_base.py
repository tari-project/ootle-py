...
The class itself is public so subclasses (``IFaucet``, future
``IAccount`` / ``IComponent``) can inherit it; the ``_``
...
class BuilderBase:
    """Shared state for :class:`IFaucet` and future template builders.
    ...
    def prepare(self) -> UnsignedTransaction:
        ...
        return self._client.resolver.resolve(unsigned, self._want_list)
