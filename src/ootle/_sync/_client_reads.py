"""Read-side helpers wrapping ``_transport`` for :class:`OotleClient`.
...
are reusable from tests without an :class:`OotleClient` in hand.
"""
...
def get_network_for_client(transport: IndexerTransport) -> Network:
    """Round-trip ``GET network`` and return the current network."""
    info = transport.get_network_info()
    return info.network


def get_epoch_for_client(transport: IndexerTransport) -> int:
    """Round-trip ``GET network`` and return the current epoch."""
    info = transport.get_network_info()
    return info.epoch


def get_substate_for_client(transport: IndexerTransport, id: SubstateId) -> Substate:
    """Return the substate; raise :class:`IndexerClientError` on 404."""
    return _get_substate(transport, id)


def fetch_substates_for_client(
    transport: IndexerTransport, ids: Sequence[SubstateId]
) -> dict[SubstateId, Substate]:
    """Batched substate fetch; chunked at 20 per HTTP call."""
    return transport.fetch_substates(ids)
...
def watch_events_for_client(
    ...
    from ootle._sync._events import watch_events as _watch  # noqa: PLC0415
