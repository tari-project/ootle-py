"""``TransactionInputResolver`` — walks a want-list, fetches substates, folds inputs.
...
from ootle._sync._resolver_helpers import (
    fold_inputs,
    resolve_all_vaults,
    resolve_specific,
...
class TransactionInputResolver:
    """Resolves a ``set[WantInput]`` against the indexer + folds inputs into the tx."""

    __slots__ = ("_transport",)

    def __init__(self, transport: IndexerTransport) -> None:
    ...
    def resolve(
        ...
        while True:
            ...
            if to_fetch:
                self._cache_substates(to_fetch, cache, missing)
    ...
    def _cache_substates(
        ...
        fetched = self._transport.fetch_substates(ids)
