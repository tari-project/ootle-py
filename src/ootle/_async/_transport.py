"""``AsyncIndexerTransport`` — the HTTP boundary class.

Owns or borrows an :class:`httpx.AsyncClient` and exposes typed wrapper
...
from ootle._async._endpoints import (
...
class AsyncIndexerTransport:
    ...
    _owns_client: bool
    _tx_watcher: AsyncTransactionWatcher | None
    ...
    async def get_network_info(self) -> NetworkInfo:
        ...
        resp = await self._request("GET", "network")
    ...
    async def fetch_substate(self, id: SubstateId) -> Substate | None:
        ...
        resp = await self._request("GET", path, allow_404=True)
        if resp is None:
            return None
    ...
    async def fetch_substates(self, ids: Sequence[SubstateId]) -> dict[SubstateId, Substate]:
        ...
        for i in range(0, len(ids), _FETCH_CHUNK):
            ...
            resp = await self._request("POST", "substates/fetch", json=payload)
            assert resp is not None  # noqa: S101 — `allow_404=False` guarantees non-None
    ...
    async def submit_transaction(self, envelope_b64: str) -> TransactionId:
        """``POST /transactions`` → :class:`TransactionId`."""
        return await submit_transaction(self, envelope_b64)

    async def submit_transaction_dry_run(self, envelope_b64: str) -> DryRunResult:
        """``POST /transactions/dry-run`` → :class:`DryRunResult`."""
        return await submit_transaction_dry_run(self, envelope_b64)
    ...
    async def get_transaction_result(
        ...
        return await get_transaction_result(self, tx_id)

    async def get_transaction_receipt(self, tx_id: TransactionId) -> TransactionReceipt | None:
        """``GET /transaction-receipts/{id}`` → :class:`TransactionReceipt` or ``None``."""
        return await get_transaction_receipt(self, tx_id)

    async def transaction_watcher(self) -> AsyncTransactionWatcher | None:
        """Lazily open (or return) the multiplexed ``/events`` SSE watcher, or ``None``."""
        return await get_transaction_watcher(self)
    ...
    async def _request(
        ...
        *,
        ...
        try:
            resp = await self._client.request(method, relative, json=json, params=params)
