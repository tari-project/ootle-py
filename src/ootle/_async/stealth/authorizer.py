"""``AsyncWalletStealthAuthorizer`` — ``Signer``-shaped stealth adapter.
...
from ootle._async.stealth._authorizer_sign import create_stealth_authorizations
from ootle._async.stealth._builder_helpers import collect_input_signers
from ootle._async.stealth._spec import StealthTransferSpec
from ootle._async.stealth._view_secret import resolve_view_secret
...
class AsyncWalletStealthAuthorizer:
    ...
    :meth:`AsyncStealthTransfer.prepare`. Call :meth:`prepare` once
    ...
    async def prepare(self, client: AsyncOotleClient) -> StealthTransferSpec:
        ...
        statement = await self._sign_balance_proof(crypto, resolution.agg_input_mask)
        await offload(lambda: crypto.validate_transfer(statement))
    ...
    async def _sign_balance_proof(
        self, crypto: StealthCryptoProvider, agg_input_mask: Mask
    ...
    async def create_authorizations(
        ...
        return await create_stealth_authorizations(self._wallet, self._spec, crypto)
    ...
    def _stealth_crypto(self, client: AsyncOotleClient) -> StealthCryptoProvider:
