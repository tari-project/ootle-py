"""Stealth-UTXO read helper consumed by :class:`AsyncOotleClient`.
...
from ootle._async._client_stealth_parse import parse_substate_utxo
...
def _require_stealth_crypto(crypto: object) -> StealthCryptoProvider:
...
class ClientStealthReadsMixin:
    """Mixin: the stealth-UTXO read method for :class:`AsyncOotleClient`.
    ...
    async def decrypt_owned_utxo(self, view_secret: bytes, utxo: Substate) -> DecryptedData | None:
        ...
        """
        return await decrypt_owned_utxo(self._crypto, view_secret, utxo)
