...
def test_get_network_round_trips(indexer_url: str) -> None:
    with OotleClient.connect(indexer_url) as client:
        network = client.get_network()
    assert isinstance(network, Network)


def test_get_epoch_returns_non_negative_int(indexer_url: str) -> None:
    with OotleClient.connect(indexer_url) as client:
...
def test_get_account_balances_returns_a_dict(indexer_url: str) -> None:
