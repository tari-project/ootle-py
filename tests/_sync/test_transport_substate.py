...
def test_fetch_substate_happy_path(httpx_mock: HTTPXMock) -> None:
    ...
    with IndexerTransport("http://idx") as transport:
        result = transport.fetch_substate(SubstateId("vault_abc"))
...
def test_fetch_substate_404_returns_none(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(
        url="http://idx/substates/component_missing",
        status_code=404,
    )
    with IndexerTransport("http://idx") as transport:
        result = transport.fetch_substate(SubstateId("component_missing"))
    assert result is None


def test_fetch_substates_chunks_at_20(httpx_mock: HTTPXMock) -> None:
    # 25 ids → 2 calls (20 + 5).
    ...
    with IndexerTransport("http://idx") as transport:
        out = transport.fetch_substates(ids)
    assert out == {}
