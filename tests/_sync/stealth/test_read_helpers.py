"""Client / wallet stealth read helpers — real WASM provider, sync surface.

Hand-written sync counterpart of
``tests/_async/stealth/test_read_helpers.py``. Drives the synchronous
:class:`~ootle.OotleClient` / :class:`~ootle.OotleWallet` stealth surface
end to end against the vendored blob — the path that was returning
un-awaited coroutines before the crypto provider was made synchronous.
"""
...
def _engine_utxo_substate(
    ...
    inner = {"output": body, "auth": {"Key": "ff" * 32}, "tag": 1}
    return Substate(
...
def test_decrypt_owned_utxo_decodes_engine_substate(
    ...
    client = make_client(httpx_mock, crypto=WasmCryptoProvider.load_default())
    utxo = _engine_utxo_substate(
        vec["commitment"], vec["sender_public_nonce"], vec["encrypted_data"]
    )
    decrypted = client.decrypt_owned_utxo(bytes.fromhex(vec["view_secret"]), utxo)
...
def test_decrypt_owned_utxo_returns_none_for_non_utxo(httpx_mock: HTTPXMock) -> None:
    ...
    assert client.decrypt_owned_utxo(b"\x11" * 32, sub) is None
    client.close()


def test_read_helpers_reject_non_stealth_crypto(httpx_mock: HTTPXMock) -> None:
    client = make_client(httpx_mock, crypto=object())
    utxo = _engine_utxo_substate("dd" * 32)
    with pytest.raises(InvalidArgumentError):
        client.decrypt_owned_utxo(b"\x55" * 32, utxo)
    client.close()
