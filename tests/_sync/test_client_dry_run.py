...
from ootle._sync._send import mark_dry_run
...
def test_send_dry_run_returns_dry_run_result(httpx_mock: HTTPXMock) -> None:
    ...
    with OotleClient.connect("http://idx", wallet=_wallet()) as client:
        result = client.send_dry_run(_unsigned_tx())
    assert result.estimated_fee == 750


def test_send_dry_run_outcome_commit(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=_NETWORK)
...
def test_send_dry_run_without_wallet_raises(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=_NETWORK)
    with OotleClient.connect("http://idx") as client:
        with pytest.raises(InvalidArgumentError):
            client.send_dry_run(_unsigned_tx())
