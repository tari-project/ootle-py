"""``IAsyncAccount`` tests — fluent shape + emitted JSON envelope."""
...
async def test_account_pay_fee_emits_fee_instruction(httpx_mock: HTTPXMock) -> None:
...
async def test_account_public_transfer_emits_correct_instructions(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
...
async def test_account_publish_template_bytes(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
...
async def test_account_publish_template_blob_wrapper(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
...
async def test_account_transfer_requires_positive_amount(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
    recipient = _recipient_key().to_address()
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
...
async def test_account_without_wallet_raises_at_default_signer(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
...
async def test_account_fluent_returns_self(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
    recipient = _recipient_key().to_address()
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
        b = client.account()
...
async def test_account_multi_transfer_unique_workspace_labels(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
...
async def test_account_want_list_populated(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
    recipient = _recipient_key().to_address()
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
