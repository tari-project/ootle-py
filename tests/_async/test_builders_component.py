"""``IAsyncComponent`` tests — instruction shapes, workspace plumbing, chain/then."""
...
async def test_call_method_emits_correct_shape(httpx_mock: HTTPXMock) -> None:
...
async def test_call_function_emits_correct_shape(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
...
async def test_call_function_accepts_bare_hex_template_address(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
...
async def test_pay_fee_lands_in_fee_block(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
...
async def test_call_method_with_literal_arg_encodes_cbor(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
...
async def test_put_last_instruction_output_on_workspace(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
        b = (
            client.component()
...
async def test_call_method_on_workspace_emits_workspace_call(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
        b = (
            client.component()
...
async def test_then_applies_callback(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
...
async def test_chain_merges_another_builder(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
        a = client.component().call_method(_COMP, "step_a")
        b = client.component().call_method(_COMP, "step_b")
        a.chain(b)
        body = build_body(a._builder)  # pyright: ignore[reportPrivateUsage]  # internal access
...
async def test_invalid_arg_raises(httpx_mock: HTTPXMock) -> None:
    mock_network(httpx_mock)
    async with AsyncOotleClient.connect("http://idx", wallet=_wallet()) as client:
