"""Shared helpers for ``AsyncStealthTransfer`` test suites."""
...
def stub_substates_for_account(httpx_mock: HTTPXMock, signer_account: str) -> None:
    """Mock indexer ``substates/fetch`` for both the synthetic ``COMPONENT`` and the signer.

    The signer's component address is derived from the wallet's default
    signer pubkey — passed in here so the resolver finds both ``COMPONENT``
    (used in tests with ``pay_fee_from_stealth`` / explicit components) and
    the auto-derived signer account (used by ``pay_fee_from_revealed`` and
    transfer flows).
    """
...
async def make_client(
    ...
    *,
    ...
    """Connect an opened :class:`AsyncOotleClient` against a mocked indexer.
    ...
    client = AsyncOotleClient.connect(
        "http://idx",
        wallet=wallet,
