...
async def test_create_authorizations_signs_each_input_with_one_time_key(
    ...
    hydrated = await AsyncWalletStealthAuthorizer.from_spec(client.wallet, spec0).prepare(client)
    ...
    auths = await create_stealth_authorizations(wallet, spec, provider)
...
async def test_create_authorizations_empty_for_revealed_only() -> None:
    ...
    assert await create_stealth_authorizations(wallet, spec, provider) == ()


async def test_create_authorizations_rejects_stealth_seal_signer() -> None:
    ...
    with pytest.raises(NotImplementedError, match="one-time stealth key"):
        await create_stealth_authorizations(wallet, spec, provider)


async def test_create_authorizations_unknown_signer_raises() -> None:
...
async def test_create_authorizations_non_stealth_signer_raises() -> None:
