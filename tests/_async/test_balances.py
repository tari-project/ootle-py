...
async def test_get_account_balance_returns_matching_resource(httpx_mock: HTTPXMock) -> None:
    ...
    async with AsyncOotleClient.connect("http://idx") as client:
        bal = await client.get_account_balance(ACCOUNT, ResourceAddress(RESOURCE_TARI))
    assert bal == Amount(100)


async def test_get_account_balance_missing_resource_returns_zero(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
    httpx_mock.add_response(
        url=f"http://idx/substates/{ACCOUNT}",
    ...
    async with AsyncOotleClient.connect("http://idx") as client:
        bal = await client.get_account_balance(ACCOUNT, ResourceAddress("resource_other"))
    assert bal == Amount(0)


async def test_get_account_balances_returns_all_vaults(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
    httpx_mock.add_response(
        url=f"http://idx/substates/{ACCOUNT}",
        json=envelope(component_value({RESOURCE_TARI: VAULT_TARI, RESOURCE_USDT: VAULT_USDT})),
...
async def test_get_account_balances_missing_account_returns_empty(httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="http://idx/network", json=network_response())
